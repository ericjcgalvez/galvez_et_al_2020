...
puls_genes <- "../genome_refs/PROD/PROD.puls.tsv"
...
type <- factor(type, levels = c("PROD-invitro", "PROD-eSPF", "PROD-Conv"))
...
set.seed(123)
pdf("./PROD_heatmap_PULs.pdf", width =8 , height = 12)
...
```{r}
prod2 <- ggplot(PUL_sums_dt_rowscaled, aes(x= variable, y = pulid)) +
...
prod <- egg::ggarrange(plots = list(prod2, prod1), nrow = 1, widths = c(0.5, 10))

# 4 x 12

g3 <- ggplot2::ggplotGrob(prod2)
g4 <- ggplot2::ggplotGrob(prod1)
