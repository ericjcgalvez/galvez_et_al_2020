...
puls_genes <- "../genome_refs/PINT/PINT.puls.tsv"
...
type <- factor(type, levels = c("PINT-invitro", "PINT-eSPF", "PINT-Conv"))
...
set.seed(123)
pdf("./PINT_heatmap_PULs.pdf", width =8 , height = 12)
...
```{r}
pint2 <- ggplot(PUL_sums_dt_rowscaled, aes(x= variable, y = pulid)) +
...
pint <- egg::ggarrange(plots = list(pint2, pint1), nrow = 1, widths = c(0.5, 10))

# 4 x 12

g1 <- ggplot2::ggplotGrob(pint2)
g2 <- ggplot2::ggplotGrob(pint1)
